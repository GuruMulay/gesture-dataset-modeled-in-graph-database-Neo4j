...
c.execute("DROP TABLE IF EXISTS participant_count_with_sound")

c.execute("""CREATE TABLE participant_count_with_sound
	 (
		 id INTEGER,
		 label text,
		 with_sound_count CHAR(20),
...
i = 1;
with open('participant_count_with_sound.csv') as tsv:
	...
	for row in csv_data:
		...
		print i
 		...
 		c.execute("INSERT INTO participant_count_with_sound (id, label, with_sound_count, participant_count) VALUES ('%s', '%s', '%s', '%s')"%(i, row[0],row[1],row[2]))
 		# if (i == 100) :
			# break;
		i +=1;
